export const getNewScaledCoords = (
	scaledCoords: any[],
	figureBottomLineYCoord: number
) => {
...
export const getRecalcTriangleSides = (scaledCoords: any[]) => {
	...
	let newASide = Math.sqrt(
		(scaledCoords[1].x - scaledCoords[0].x) ** 2 +
			(scaledCoords[1].y - scaledCoords[0].y) ** 2
	)
	...
	let newBSide = Math.sqrt(
		(scaledCoords[2].x - scaledCoords[1].x) ** 2 +
			(scaledCoords[2].y - scaledCoords[1].y) ** 2
	)
	...
	let newCSide = Math.sqrt(
		(scaledCoords[0].x - scaledCoords[2].x) ** 2 +
			(scaledCoords[0].y - scaledCoords[2].y) ** 2
	)
...
export const getRecalcTrapezoidSides = (scaledCoords: any[]) => {
	if (!scaledCoords || scaledCoords?.length !== 4)
	...
	let newCSide = Math.sqrt(
		(scaledCoords[3].x - scaledCoords[2].x) ** 2 +
			(scaledCoords[3].y - scaledCoords[2].y) ** 2
	)
	newCSide = +newCSide.toFixed(2)
	newCSide = newCSide / 100
	...
	let newDSide = Math.sqrt(
		(scaledCoords[0].x - scaledCoords[3].x) ** 2 +
			(scaledCoords[0].y - scaledCoords[3].y) ** 2
	)
...
export const getFigureHeight = (scaledCoords: any) => {
	const result_bottom = scaledCoords.reduce((a: any, b: any) => {
		return a.y < b.y ? a : b
	})
	const result_top = scaledCoords.reduce((a: any, b: any) => {
		return a.y > b.y ? a : b
	})
