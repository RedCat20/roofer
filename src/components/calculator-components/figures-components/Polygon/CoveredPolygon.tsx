...
const CoveredPolygon: FC<Props> = ({cellSize, coords, gridHeight, startCoords,dictionaryItem,setCalcResult}) => {
    ...
    function recalcCoords() {
        ...
        const result = scaledCoords.reduce((a,b)=>{ return (a.y < b.y) ? a : b })
        ...
        if (scaledCoords?.length > 1) {
            ...
            dictionaryItem.made.forEach((item: any) => {
                ...
                if ((leave >= Number(arrSplit[0]) && leave <= Number(arrSplit[1])) || (leave < Number(arrSplit[0]))) {
                    // value = leave * scalesConfig[`${appContext.state.selectedScale}`];
            ...
            //     <Line key={`covered-rectangle-horizontal-overlap-${Math.random()}`}
            //           x={cellSize}
            //           y={figureBottomLine + step2 + step2}
            //           width={1}
            //           points={[0, 0, 1100, 0, 0]}
            //           stroke="red"
            //     />
            ...
            //     <Line key={`covered-rectangle-horizontal-overlap-${Math.random()}`}
            //           x={cellSize}
            //           y={figureBottomLine + step2 + step2 + value}
            //           width={1}
            //           points={[0, 0, 1100, 0, 0]}
            //           stroke="red"
            //     />
                ...
                for (let i = cellSize; i < 1100; i += step1) {
                    ...
                    for (let a = 0; a < step1; a++) {
                        ...
                        for (let b: number = 0; b < step2; b++) {
                           ...
                           // let isPointInPath = ctx.isPointInPath(((i + a) * scalesConfig[`${appContext.state.selectedScale}`]), -((j + b) * scalesConfig[`${appContext.state.selectedScale}`]));
                            let isPointInPath = ctx.isPointInPath(((i + a) * scalesConfig[`${appContext.state.selectedScale}`]), -((j + b) * scalesConfig[`${appContext.state.selectedScale}`]));
                            ...
                            if (isPointInPath) {
                                ...
                                if (cyrcle_counter < 3) {
                                    ...
                                    // arr.push(
                                    //     <Line key={`covered-rectangle-horizontal2-overlap-${Math.random()}`}
                                    //           x={i}
                                    //         //  y={j + b}
                                    //         //  y={((j + b) * scalesConfig[`${appContext.state.selectedScale}`])}
                                    //           y={(j + b)}
                                    //           width={1}
                                    //           points={[0, 0, step1, 0, 0]}
                                    //           stroke="violet"
            ...
            setCalcResult(`
                <div>
                   <br/>
                    ...
                    <br/>
                    Листи 3 ряду: ${top_row_blocks_count} шт - ${step1 / 100} м x ${value / 1000} м;
                     <br/>
                 </div>
                 <br/>
                 <br/>
