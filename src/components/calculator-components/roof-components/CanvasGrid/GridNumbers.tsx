...
const GridNumbers: FC<Props> = ({cellSize,gridHeight,gridHorizontalNumbers, gridVerticalNumbers}) => {
            ...
            arr.push(
                <Text text={i.toString()} key={1000 + i}
                     ...
                     // offsetX={-appContext.state.gridConfig.cellSize}
                      y={- gridHeight / scalesConfig[`${appContext.state.selectedScale}`] + 10}
                      scaleY={-1}
                      fill={'red'}
                      width={100}
                />
        ...
        for (let i = 0; i < 50; i++) {
            ...
            arr.push(
                <Text text={(i).toString()} key={10000 + i}
                      ...
                      scaleY={-1}
                      fill={'red'}
                      y={-gridHeight / scalesConfig[`${appContext.state.selectedScale}`] + newY}
                      width={100}
                />
            );
        }
