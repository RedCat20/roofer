...
const CoveredSquare: FC<Props> = ({ dictionaryItem, setCalcResult }) => {
	...
	const {
		figureSides: { figureASide: _figureSide },
	...
	useEffect(() => {
		if (_figureSide && dictionaryItem) {
			calcBlockHeights()
		}
	}, [_figureSide, dictionaryItem])

	const calcBlockHeights = () => {
		const _mainBlock = mm_to_m(
			Math.max(
				...dictionaryItem.recommended.filter(
					(v: number) => v <= m_to_mm(_figureSide / 2)
				)
			)
		)
		setMainBlockHeight(_mainBlock)

		let _leaveHeight = _figureSide - _mainBlock * 2 + OVERLAP * 2
	...
	function createCellRows() {
		...
		const width = _figureSide * cellSize
		...
		let square_s = (_figureSide * _figureSide).toFixed(2)
					...
					<Layer>
						<Rect
							...
							y={
								...
								_figureSide * gridConfig?.cellSize
							}
							width={_figureSide * gridConfig?.cellSize}
							height={_figureSide * gridConfig?.cellSize}
							// fill='gold'
							// opacity={0.5}
							strokeWidth={3}
							stroke={'black'}
